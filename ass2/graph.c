...
// the following code has been adapted from the lecture code 

typedef struct GraphRep {
  int **edges; // 2d array of edges 
...
{
  assert(numVertices >= 0); // check if the vertex is greater than or equal to 0 
  ...
  {
    g->edges[i] = calloc(numVertices, sizeof(int)); // this will allocate memory and set every element to zero meaning that nothing is inserted yet   
  }
  return g;  
}
...
void GraphEdgeInsert(Graph g, Edge e)
{
  if (!g->edges[e.v][e.w]) // if (g->edges[e->v][e->w] == 0) which means if edge e isn't in graph 
  {
    ...
    g->edges[e.w][e.v] = 1; // the reason why we do this twice is because there is an edge from v - w and w - v (undirected graph) so we set it as 1 twice 
...
{
  if (g->edges[e.v][e.w]) // if g->edges[e->v][e->w] == 1) which means if the edge is found 
  {
...
void GraphShow(Graph g)
{
  ...
  {
    for (int j = i + 1; j < g->nV; j++) // loop through the cols 
    {
      if (g->edges[i][j]) // if (g->edges[i][j] == 1) meaning if edge found then go ahead and print 
      {
        printf("Edge %d - %d\n", i, j);
      }
    }
...
int GraphCheckAdjacent(Graph g, Vertex v, Vertex w)
{
  return g->edges[v][w]; 
}